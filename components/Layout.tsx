...
export const Layout: React.FC<LayoutProps> = ({ user, activeTab, onTabChange, onLogout, children, onPostCreated }) => {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50 max-w-md mx-auto shadow-2xl relative overflow-hidden border-x border-gray-200">
      ...
      {activeTab === 'home' && (
          <header className="bg-white px-4 py-3 flex items-center justify-between border-b border-gray-100 z-20 sticky top-0">
            <div className="flex items-center gap-2 cursor-pointer" onClick={onLogout}>
                <h1 className="text-xl font-bold text-teal-600 font-sans tracking-tight">MedGram</h1>
            </div>
